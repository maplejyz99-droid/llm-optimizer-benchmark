...
PYTHONDONTWRITEBYTECODE=1 python3 ./src/main.py \
  ...
  --experiment_name remote-softeq-cpu-smoke \
...
迁移到 `/root/work/llm-optimizer-benchmark` 后已再次运行同等 Mac CPU smoke，实验名为 `remote-softeq-cpu-smoke`，确认新 import path 能完成 dataset load、model build、optimizer build、2 step training 和 `summary.json` 生成。
