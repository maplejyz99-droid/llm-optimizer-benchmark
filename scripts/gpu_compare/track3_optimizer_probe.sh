...
OPTS="${OPTS:-softeq-k2000-muon muon sophiag}"
...
if (( WARMUP_STEPS < 2 )); then
    echo "ERROR: WARMUP_STEPS must be >= 2 for this repo's wsd/cos scheduler probes." >&2
    exit 2
fi
...
optimizer_args() {
    case "$1" in
        ...
        sophiag)
            ...
            ;;
        newton-muon)
            echo "ERROR: newton-muon is single-card only in src/main.py and cannot be run as DDP/world=${NPROC_PER_NODE}." >&2
            return 2
            ;;
...
run_one() {
    ...
    run_name="gpu-${GPU_LABEL}_track3-probe_model-${MODEL_PRESET}_seq-${SEQUENCE_LENGTH}_effbs-${TARGET_EFF_BATCH}_microbs-${microbs}_acc-${acc_steps}_world-${NPROC_PER_NODE}_opt-${opt}_seed-${SEED}_steps-${ITERATIONS}${suffix}"
