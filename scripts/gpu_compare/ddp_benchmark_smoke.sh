...
OPTS="${OPTS:-softeq-k2000-muon adamw muon sophiag}"
...
if (( WARMUP_STEPS < 2 )); then
    echo "ERROR: WARMUP_STEPS must be >= 2 for this repo's cos/linear OneCycleLR scheduler." >&2
    exit 2
fi
...
optimizer_args() {
    ...
    case "${opt}" in
        ...
        sophiag)
            ...
            ;;
        newton-muon)
            echo "ERROR: newton-muon is single-card only and is intentionally excluded from DDP smoke." >&2
            return 2
            ;;
...
run_probe() {
    ...
    run_name="gpu-${GPU_LABEL}_ddp-smoke_case-${case_name}_model-${model}_seq-${seq}_effbs-${effbs}_microbs-${microbs}_acc-${acc_steps}_world-${NPROC_PER_NODE}_opt-${opt}_seed-${SEED}_steps-${run_iterations}${suffix}"
